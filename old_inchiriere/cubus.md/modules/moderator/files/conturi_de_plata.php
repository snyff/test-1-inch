...
// Last modification date:  16.12.2008                                               //
// Version:                 CUBUS  0.3b                                              //
//                                                                                   //
...
///////////////////////////////////////////////////////////////////////////////////////
	...
	if ( me('id') && !$GLOBALS["CHROMELESS_MODE"] ) {	
		...
		$tpl -> Zone("contDePlataModul", "enabled");
		$tpl -> Zone("filesBlock", "enabled");
 	}  
...
########################################################
	...
	if ( me('id')!='' ) {	
	    ...
	    if ( $_GET['selectEditor']!='' && $_GET['fileEditor'] != '' ) {
			 
			$selectFiledataSelect = myF(myQ("
				SELECT * 
				FROM `[x]files` 
				WHERE `id`='".$_GET['fileEditor']."'
			"));
			
			 
			 if ( $_GET['selectEditor']=='-' ) { $approve_data = '8'; } 
			 else                              { $approve_data = '9'; } 
			 
			 
			 myQ("UPDATE `[x]files` 
			 	  SET 
				  		`ideditor`='".$_GET['selectEditor']."',  
				  		`approved`='".$approve_data."',  
				  		`salariu_editor`='"._fnc("salariuEditori", $selectFiledataSelect['pretul'], $_GET['selectEditor'])."'
 		...
 		while ($fisierList = myF($fileSelect)) {
			...
			*/
		    ...
		    /*
			           Lista de traducatori
			*/
		    $fileSelectEditor = myQ("
			           SELECT *
			           FROM `[x]traducatori`
					   WHERE `editor_active` = '1'
 			           ORDER BY `id_traducator` DESC
		    ");
			
		    $dataEditor = $fisierList['from_lang']."-".$fisierList['to_lang']; 
			
 		    $dataeditor='';
			
			while ($fisierListEditor = myF($fileSelectEditor)) {
			
				   $selectAbilitati = unpk($fisierListEditor["abilitati"]);
				   
				   $numePrenume = explode(".", _fnc("user", $fisierListEditor["id_traducator"], "username"));
			  
				   if ( $selectAbilitati[$dataEditor]['statut_file'] == 4 ) {
					  
					   //echo $fisierList['fortraduc'] . '=======' . $fisierListEditor['id_traducator']; echo "<BR>";
					   
					   if ($fisierList['ideditor'] == $fisierListEditor['id_traducator'] ) {
					   
						   $dataeditor .= "<option selected value=\"".$fisierListEditor['id_traducator']."\">".ucwords($numePrenume[0])." ".ucwords($numePrenume[1])."</option>\r\n";
					   }
					   else {
						   
						   $dataeditor .= "<option value=\"".$fisierListEditor['id_traducator']."\">".ucwords($numePrenume[0])." ".ucwords($numePrenume[1])."</option>\r\n";
					   }
				   }
			}
			...
			$selectEditor = "<select name=\"traducator\"  onchange=\"document.location.href='http://cubus.md/?L=moderator.conturi_de_plata&fileEditor=".$fisierList['id']."&selectEditor=' + this.value;\">
						       <option>-</option>
							   ".$dataeditor."
						   </select>";
			...
			*/
		    ...
		    $fisier[$i]["data.editor"]     = $selectEditor;

            
			/* Separam fisierele aprobate de cele neaprobate */
			    if ( $fisierList['approved'] >= 5 && $fisierList['approved'] <= 7 )  {  $fisiereTraducere[] = $fisier[$i]; }
			elseif ( $fisierList['approved'] >= 8 && $fisierList['approved'] <= 10 ) {  $fisiereEditare[]   = $fisier[$i]; }
