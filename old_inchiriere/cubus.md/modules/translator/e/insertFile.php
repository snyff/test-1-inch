...
########################################################
	...
	if (isset($_POST["Submit"]) && me("id")!='') {
  		...
  		$testData = myF(myQ("SELECT * FROM `[x]editors_translate` WHERE `id_et`='".$_POST['id']."' LIMIT 1"));
		
		if ($testData['test_original_file']!='') {	
			_fnc("reload", 0, "?L=translator.e.tests&error=fileExist");
			die;
		}
		...
		if ( $_FILES['file']['size'] <= $CONF["MAX_UPLOAD_FILE_SIZE"] ) {
			...
			*/
				   		...
				   		_fnc("reload", 0, "?L=translator.e.tests&error=noFile");
				  }
				  
		// file big then error
		} else  {

					_fnc("reload", 0, "?L=translator.e.tests&error=BigFileSize");
			    }
			  
	} else {
	
	 	_fnc("reload", 0, "?L=translator.e.tests&error=unKnown");
	}
