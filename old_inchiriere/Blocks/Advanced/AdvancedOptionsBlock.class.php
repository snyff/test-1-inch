	...
	function buildContent() {
		...
		$sendButton = '<td valign="bottom">
							<table>
								...
								<tr>
									<td>
										<img src="'.getUrl(array('pagePath'=>'CaptchaImg', 'lang'=>LANG, 'queryString'=>array('t',time()))).'" id="captcha" class="cursorPointer" onClick="document.getElementById(\'captcha\').src=\' '.getUrl(array('pagePath'=>'CaptchaImg', 'lang'=>LANG)).'?dd=\'+new Date().getTime();" alt="Click to refresh">
									</td>
								</tr>
							</table>
					   </td>
					   <td valign="bottom" class="paddingBottom5">
					   		'.but(translate(52, LANG, 'Trimite'), 'button', 'onClick="validate('.$jsVars.')"').'
					   </td>
					   ';
		$js = '
			<script>
				...
				function ajaxRequestSerialize( action, form_id ) {
					$.ajax({
						url:	"'.getUrl(array('pagePath'=>'AdvancedOptions', 'absolute'=>false, 'reste'=>'submit', 'lang'=>LANG)).'?action=" + action + "&" + $(form_id).serialize(),
						beforeSend: function(){
						},
		...
		$html .= '
			<div class="paddingLeft5 paddingRight5">
				<div class="displayRight marginRight10"><a href="javascript:void(0);" class="textBold" onClick="removeAdvancedDiv();">'.translate(53, LANG, 'Inchide').'</a></div>
				...
				<form action="" method="get" id="content_form">
					<table border=0>
						<tr>
							...
							'.$sendButton.'
							<td valign="top" id="result" class="colorRed textBold">
							</td>
						</tr>
					</table>
				</form>
