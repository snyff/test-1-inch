...
########################################################
	...
	if (isset($_POST["Submit"]) && me("id")!='') {
  		...
  		$testData = myF(myQ("SELECT * FROM `[x]translator_test_files` WHERE `translation_languages_id`='".$_POST['translation_languages_id']."' LIMIT 1"));
		
		if ($testData['id_ttf']!='') {	
			_fnc("reload", 0, "?L=moderator.translators.translatorsTests&error=fileExist");
			die;
		}
		...
		if ( $_FILES['file']['size'] <= $CONF["MAX_UPLOAD_FILE_SIZE"] ) {
			...
			if (is_uploaded_file($_FILES["file"]["tmp_name"])) {
				...
				if (strstr(basename($_FILES["file"]["name"]), ".")) {
					...
					if (in_array($fileExtention, explode(",", $CONF["PICTURES_ALLOWED_EXTENTIONS"]))) {						
						...
						/* Save to database */
						myQ("INSERT INTO `[x]translator_test_files` (
						...
						_fnc("reload", 0, "?L=moderator.translators.translatorsTests&data=success");
 					}				
					/* File extension is not contained in allowed list */
					else  {
	
							_fnc("reload", 0, "?L=moderator.translators.translatorsTests&error=unallowedExtension");
						  }
				}	
				/* File extension is not contained in allowed list */
				else  {
			...
			*/
					...
					_fnc("reload", 0, "?L=moderator.translators.translatorsTests&error=noFile");
				  }
				  
		// file big then error
		} else  {

				_fnc("reload", 0, "?L=moderator.translators.translatorsTests&error=BigFileSize");
			  }
			  
	} else {
	
		_fnc("reload", 0, "?L=moderator.translators.translatorsTests&error=unKnown");
	}
