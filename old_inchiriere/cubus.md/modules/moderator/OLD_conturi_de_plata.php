...
// Last modification date:  05.01.2007                                               //
// Version:                 CUBUS  0.1b                                              //
//                                                                                   //
...
########################################################
	...
	if ( me('id') ) {	
	    ...
	    if ( $_GET['selectTraducator']!='' && $_GET['fileTraducator'] != '' ) {
		     ...
		     myQ("UPDATE `[x]files` SET `fortraduc`='".$_GET['selectTraducator']."'  WHERE `id`='".$_GET['fileTraducator']."'");	
			       ...
			       */
				   ...
				   while ($fisierListTrad = myF($fileSelectTrad)) {
			           ...
			           if ( in_array($dataTrad, $arrayAllTested) ) {
						   ...
						   if ($fiecareFisierSeparat['fortraduc'] == $fisierListTrad['id'] ) {
						   
						       $datatraducator .= "<option selected value=\"".$fisierListTrad['id']."\">".ucwords($numePrenume[0])." ".ucwords($numePrenume[1])."</option>\r\n";
						   }
						   else {
						   
						       $datatraducator .= "<option value=\"".$fisierListTrad['id']."\">".ucwords($numePrenume[0])." ".ucwords($numePrenume[1])."</option>\r\n";
						   }
					   }
			       }
			...
			while ($fisierListTrad = myF($fileSelectTrad)) {
			      ...
			      $arrayAllTested = array();
			           ...
			           if ( in_array($dataTrad, $arrayAllTested) ) {
						   ...
						   if ($fisierList['fortraduc'] == $fisierListTrad['id'] ) {
