...
///////////////////////////////////////////////////////////////////////////////////////
	...
	class template {
		...
		function Load($file) {
			...
			if ($strict) {
				if (is_file($loadFile)) $this->Buffer = str_replace("==blocklist==", file_get_contents("theme/default/add_elements/blocks.tpl"),  file_get_contents($loadFile));
				else {
				...
				if (is_file("theme/{$GLOBALS["THEME"]}/templates/{$loadFile}")) {
					$this->Buffer = str_replace("==navbar==", file_get_contents("theme/default/add_elements/navbar.tpl") , str_replace("==blocklist==", file_get_contents("theme/default/add_elements/blocks.tpl"), file_get_contents("theme/{$GLOBALS["THEME"]}/templates/{$loadFile}")));
		...
		// ASSIGN LOOP ////////////////////////////////////////////////////////////////
		function Loop($name, $array, $newline=array() ) {
			global $CONF;
			...
			if (isset($tag[1])) {
				...
				foreach($array as $key => $arr) {
					...
					if ( is_array($newline) && $yep % 2 === 0 ) 
					   { 
					      $newline_start = $newline[0];  
					      $newline_finish = $newline[1];  
					   }
					elseif ( is_array($newline) )
					   { 
					      $newline_start = $newline[2];  
					      $newline_finish = $newline[3];  
					   }
					/* /Adaugat de mine */
					...
					if (is_array($arr)) {
						...
						$ret .= $newline_start . $temp . $newline_finish;
 					}
				}
		...
		function Translate() {
			...
			if (!$CONF["TRANSLATOR_FLAT_MODE"]) {
				...
				if ( $CONF["LOCALE_LANGUAGE_ALLOW_OVERRIDE"] and me("id")=='' and $_SESSION['currentlang'] !='' ) {
					
					$dictionaryFile = $CONF["LOCALE_LANGUAGEPACK_LOCATION"]."/".$_SESSION['currentlang'].".php";
					$language = $_SESSION['currentlang'];
									
				} elseif ($CONF["LOCALE_LANGUAGE_ALLOW_OVERRIDE"] and me("language")) {
	            ...
	            /*
				    be me //---> Eu am comentat rindul de mai jos
				*/
