	...
	if ( me('id') && !$GLOBALS["CHROMELESS_MODE"] ) {	
 	...
 	if ( me('id') ) {	
		...
		while ($select = myF($selectL)) {
			...
			$testData = myF(myQ("SELECT * FROM `[x]translator_test_files` WHERE `translation_languages_id`='".$select['id']."' LIMIT 1"));
			
			$add_file  = array("{translation.languages.id}" => $select['id']);
			$file_data = array(
				"{file.name}" => $testData['original_name'],
				"{file.id}"   => $testData['id_ttf']
			);
			
			$LList[$i]["file"]           = ($testData['id_ttf']=='')?strtr($GLOBALS["OBJ"]["fileNone"], $add_file):strtr($GLOBALS["OBJ"]["fileIs"], $file_data);
