...
$tpl -> GetObjects(); 
	...
	/* CRON - trebuie sa fie aici din motiv ca se face CRON de fiecare data cind se intra in acest modul */
	...
	if ( me('id') ) {	
		...
		$selectL = myQ("SELECT * FROM `[x]files` WHERE `status_file`='0' OR `status_file`='10' OR `status_file`='20' ORDER BY `file_id` DESC");		
		$i=0;
		while ($files = myF($selectL)) {
			...
			/* Separam fisierele aprobate de cele neaprobate */
				if ( $files['status_file'] == 0 )  {  $newFiles[]      = $filesArray[$i]; }
